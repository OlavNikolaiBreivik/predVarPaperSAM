...
plotit.sam <- function(fit, what, x=fit$data$years, ylab=what, xlab="Years", ex=numeric(0), trans=function(x)x, add=FALSE, ci=TRUE, cicol=gray(.5,alpha=.5),
                   addCI=NA, drop=0, unnamed.basename="current", xlim=NULL,ylim = NULL,lwd = 3,ylabInternal = NULL,...){
    ...
    if(!is.null(ylabInternal))ylab = ylabInternal

    if(add){
      lines(x, trans(y), lwd=lwd,...)
    }else{
      if(is.null(ylim)){
        plot(x, trans(y), xlab=xlab, ylab=ylab, type="n", lwd=lwd, xlim=xr, ylim=range(c(trans(lowhig),0,ex)), las=1,...)
      }else{
        plot(x, trans(y), xlab=xlab, ylab=ylab, type="n", lwd=lwd, xlim=xr, ylim=ylim, las=1,...)
      }
      grid(col="black")
      lines(x, trans(y), lwd=lwd, ...)
    }
    if(ci){
      ...
      lines(x, trans(y), lwd=lwd, col=cicol)
      lines(x, trans(y), lwd=lwd*2/3, col="black", lty="dotted")
    }
}
##' @rdname plotit
...
plotby <-function(x=NULL, y=NULL, z=NULL, x.line=NULL, y.line=NULL, z.line=NULL, by=NULL,  bubblescale=1, x.common=!is.null(x), y.common=!is.null(y), z.common=!is.null(z), xlab=NULL, ylab=NULL, xlim=NULL, ylim=NULL, zmax=NULL, axes=TRUE, ...){
  ...
  if(is.vector(by)|is.factor(by)){
    ...
    if(is.null(zmax) & z.common) zmax <- max(sqrt(abs(z)), na.rm=TRUE)
